...
const DriversView = () => {
    ...
    const [rows, setRows] = useState<(Product | Driver | OrderRow | Customer | Route | Receipt)[]>([]);
    const [paginationModel, setPaginationModel] = useState<GridPaginationModel>({page: 0, pageSize: 10})
    const [rowCount, setRowCount] = useState(0);
    const [loading, setLoading] = useState(false);
    const [openEdit, setOpenEdit] = useState(false);
    const [rowToEdit, setRowToEdit] = useState<OrderItem | Driver | Product | Customer | Route | Receipt | undefined>();
    ...
    const handleUpdateDriver = (updated: Product | Driver | OrderRow | Customer | Route | Receipt) => {
        setRows(prev => {
