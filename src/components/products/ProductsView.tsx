...
import type {Customer, Driver, OrderItem, OrderRow, Product, Receipt, Route} from "../../types/Types.ts";
...
const ProductsView = () => {
    const [rows, setRows] = useState<(Product | Customer | OrderRow | Driver | Route | Receipt)[]>([]);
    const [paginationModel, setPaginationModel] = useState<GridPaginationModel>({page: 0, pageSize: 10})
    const [rowCount, setRowCount] = useState(0);
    const [loading, setLoading] = useState(false);
    const [openEdit, setOpenEdit] = useState(false);
    const [rowToEdit, setRowToEdit] = useState<Product | Customer | OrderItem | Driver | Route | Receipt | undefined>();
    ...
    const handleUpdateProduct = (updated: Product | Customer | OrderRow | Driver | Route | Receipt) => {
        setRows(prev => {
